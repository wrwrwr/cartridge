...
class Voucher(Discount):
    """
    Similar to discount code, but with single-use autogenerated codes.
    """
    min_purchase = fields.MoneyField(_("Minimum total purchase"))
    free_shipping = models.BooleanField(_("Free shipping"))
    ...
    def new_code(self):
        ...
        The returned ``VoucherCode`` object is prestored and may be passed
        to a template right away. We only try drawing a nonexisting code a
        couple of times in case one chooses a short code length.
        """
        for i in range(100):
            code = get_random_string(
                length=settings.SHOP_VOUCHER_CODE_LENGTH,
